...
namespace ListenService.Repository.Implements
{
    public class EbaySetStatus : IEbaySetStatus
    {
        ...
        )
        {
            ...
            _isRunning = true;
            Console.WriteLine(
                DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "EbaySetStatus程序启动：" + chainId
            );
            ...
            catch (Exception ex)
            {
                Console.WriteLine(
                    DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")
                        + $"EbaySetStatus启动异常:{ex} - Chain ID: {chainId}"
                );
                _isRunning = false;
        ...
        )
        {
            ...
            Console.WriteLine(
                DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "EbaySetStatus监听到了！" + chainId
            );
            ...
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                //await Task.Delay(2000);
                // 获取智能合约中的订单信息
                var orderResult = await function.CallDeserializingToObjectAsync<EbayOrderDTO>(
                    (int)decoded.Event.OrderId
                );
                var chainToken = _masterDbContext.chain_tokens.FirstOrDefault(a =>
                    a.token_address.Equals(orderResult.Token) && a.chain_id == chainId
                );

                if (chainToken != null)
                {
                    var decimalsNum = new BigDecimal(Math.Pow(10, chainToken.decimals));
                    var order = _masterDbContext.orders.FirstOrDefault(a =>
                        a.order_id == (int)decoded.Event.OrderId
                        && a.chain_id == chainId
                        && a.contract.Equals(contractAddress)
                    );
                    ...
                    if (order != null)
                    {
                        ...
                        order.buyer_ex = (double)(
                            new BigDecimal(orderResult.BuyerEx) / decimalsNum
                        );
                        order.update_time = DateTime.Now;
                        order.buyer = orderResult.Buyer;
                        order.buyer_pledge = (double)(
                            new BigDecimal(orderResult.BuyerPledge) / decimalsNum
                        );
                        order.seller_pledge = (double)(
                            new BigDecimal(orderResult.SellerPledge) / decimalsNum
                        );
                        order.amount = (double)orderResult.Amount;
                        order.price = (double)(new BigDecimal(orderResult.Price) / decimalsNum);
                        order.seller_ratio = (decimal)(
                            order.seller_pledge / (order.price * order.amount)
                        );

                        _masterDbContext.SaveChanges();

                        // 发送消息
                        _ = _sendMessage.SendMessageEbay(
                            (int)decoded.Event.OrderId,
                            chainId,
                            contractAddress
                        );
        ...
        private async Task CleanupResources()
        {
            ...
            {
                Console.WriteLine(
                    DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + $"EbaySetStatus清理资源异常:{ex}"
                );
            }
        }
    }
