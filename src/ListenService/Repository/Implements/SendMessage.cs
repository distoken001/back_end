            ...
            try
            {
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    if (status == OrderStatus.Initial)
                    {
                        ...
                        var chatMessage = $"担保订单（{yajin}）：用户 @{seller?.nick_name} 在{order.chain_id.ToString()}链上发布了新商品：{order.name}，单价：{order.price.ToString("R")} {token.token_name}, 数量：{order.amount}，订单链接:{_configuration["Domain"]}/market/detail/{order.contract}/{(int)order.chain_id}/{order.order_id}";
