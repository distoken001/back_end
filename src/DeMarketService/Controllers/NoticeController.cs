    ...
    public class NoticeController : BaseController
    {
        ...
        public async Task<JsonResult> SendEmail([FromBody] SendEmailRequest request)
        {
            ...
            try
            {
                ...
                {
                    mailMessage = $"您在{order.chain_id.ToString()}发布的商品({order.name})已经上架，如果您的商品有新动态，我们将邮件通知您！";
                }
                if (status != OrderStatus.Initial)
                {
                    mailMessage = $"您在{order.chain_id.ToString()}的商品（{order.name}）有新动态，请注意查看！<br/><br/><br/>------此邮件收件人为买卖双方预留的邮箱联系方式，您也可以在商品详情页查看对方的其他联系方式！";
                }
